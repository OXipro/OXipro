# 👋 Hey, I'm OXipro

---

## 🚀 About Me

I'm **OXipro**, a developer passionate about many things as long as it touches IT.

Minecraft is what got me into programming. It's where I learned **Java** (tried), and started building plugins and backend systems.

Today, I mainly develop around the PaperMC ecosystem and enjoy building performant, clean solutions. (not always lmao)

---

## 🛠️ Main Technologies

<p align="center">
  <img src="https://skillicons.dev/icons?i=java,python,docker,redis,git,idea,vscode" />
</p>

### Minecraft Development

* ⚡ Paper
* 🧩 Spigot
* 🌐 Velocity
* ☕ Java
* 📦 Maven

### Backend & Infrastructure

* 🐳 Docker
* ☸️ Currently learning Kubernetes
* 🔴 Redis
* 🗄️ Basic MySQL knowledge (bare minimum)
* 🔧 Git & GitHub (bare minimum)

### Other Languages

* 🐍 Python for smal projets

---

## 💻 Development Environment

<p align="center">
  <img src="https://skillicons.dev/icons?i=idea,vscode,arch,debian,ubuntu" />
</p>

### Editors

* IntelliJ IDEA
* Visual Studio Code

### Operating Systems

* 🐧 Arch Linux (I use arch BTW)
* EndeavourOS (main desktop)
* CachyOS
* Ubuntu / Debian (servers)

### Virtualization & Homelab

* Proxmox (currently learning and using)
* VMware ESXi (previous experience)

---

## 🔌 Embedded & Home Automation

I also enjoy building embedded projects.

I've worked with:

* Arduino
* ESP8266 / ESP32
* MQTT
* Home Assistant

Mostly for smart home and home automation projects.

---

## 🖥️ Hardware

I'm also quite comfortable with PC hardware:

* Building PCs
* Choosing components
* Troubleshooting
* General hardware knowledge

---

## 📊 GitHub Stats

<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=OXipro&show_icons=true&theme=tokyonight&hide_border=true"/>
  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=OXipro&layout=compact&theme=tokyonight&hide_border=true"/>
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=OXipro&theme=tokyonight&hide_border=true"/>
</p>

---

## 🎯 Currently Learning

* ☸️ Kubernetes
* 🏗️ Better infrastructure & orchestration
* 📦 Improving Docker workflows
* ⚡ Scaling Minecraft networks

---

## 💬 Fun Facts

* ❤️ Minecraft has been my biggest source of motivation to learn programming.
* 🐧 Arch Linux user, by the way.
* 🏠 I enjoy experimenting with homelabs and self-hosted services.
* 🔧 I like understanding both software and hardware.

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=OXipro&color=blueviolet&style=for-the-badge"/>
</p>
